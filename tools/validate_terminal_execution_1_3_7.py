...
expect('Version="1.3.7"' in manifest,'manifest version mismatch')
expect('"1.3.7"' in package,'InstalledProductRegistration version mismatch')
pack=json.loads((PROJ/'Assets/Packs/system_scripting_pack.json').read_text(encoding='utf-8'))
expect(pack.get('version')=='1.8.0',f'system scripting pack version mismatch: {pack.get("version")!r}')
...
for required in ['ssh login with password prompt','ssh remote command','ssh-copy-id install public key','Reference — SSH password login and secret handling']:
...
summary={'visualStudioSourceVersion':'1.3.7','systemScriptingPackVersion':pack.get('version'),'executeButton':True,'externalConsoleRouting':True,'integratedPackGenerationAdded':False,'errors':errors}
